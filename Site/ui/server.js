...
function cacheSiteData(req,res) {
  client
  .query({
    query: gql `
      {
    ...
    `
      ...
      //Make sure to add pages not instantiated in the CMS. In this sample, blog page is not created in the CMS
      pages.push('blog');
      ...
      for(var i = 0; i < pages.length; i++) {
        if(pages[i] == 'home') {
          ...
          renderAndCache(req,res,'/'+pages[i]);
        }
      }

      //Cache any posts
      for(var i = 0; i < posts.length; i++) {
        renderAndCache(req,res,'/blog/'+posts[i]);
      }
  })
}